...
#define BIT_CLR(reg, n)   (reg &= (~(UINT32_C(1) << (n))))
#define BIT_TOG(reg, n)   (reg ^= (UINT32_C(1) << (n)))
...
int main(void) {
  ...
  printf("%s ->  0x%08x == 0x00000020\n", 
    (BIT_CLR(*sample, 1) == 0x00000120)? "PASS" : "FAIL", BIT_CLR(*sample, 1));
  printf("%s -> got 0x%08x\n", 
    (BIT_TOG(*sample, 8) == 0x00000020)? "PASS" : "FAIL", BIT_TOG(*sample, 8));
  return 0;
}
